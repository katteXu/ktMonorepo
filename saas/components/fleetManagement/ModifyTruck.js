...
const ModifyTruck = ({
  ...
  <Form layout="horizontal" labelAlign="right" labelCol={{ span: 5 }} wrapperCol={{ span: 10 }}>
    <Form.Item label="所属车队">
      <span>{fleetName}</span>
    </Form.Item>
    <Form.Item label="车牌号：">
      {getFieldDecorator('trailerPlateNumber', {
        validateFirst: true,
        initialValue: trailerPlateNumber,
        rules: [
          {
            required: true,
            message: '请输入车牌号!',
    ...
    <Form.Item
      label={
        <div>
          <span style={{ display: 'inline-block', marginRight: 4, visibility: 'hidden' }}>*</span>行驶证：
        </div>
      }
      // label="行驶证："
    >
      {getFieldDecorator('truckLicenseNumber', {
        initialValue: truckLicenseNumber,
      })(<Input placeholder="请输入行驶证" />)}
    </Form.Item>
    <Form.Item label="从业资格证：">
      {getFieldDecorator('qualificationNumber', {
        validateFirst: true,
        initialValue: qualificationNumber,
        rules: [
          {
            required: true,
            message: '请输入从业资格证!',
...
export default Form.create()(ModifyTruck);
