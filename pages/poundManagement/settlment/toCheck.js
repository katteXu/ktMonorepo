...
import { EditPriceForm } from '@components/pound/Settlment';
...
// 编辑单元格
const EditCell = ({ title, editable, children, dataIndex, reload, record, ...restProps }) => {
  ...
  const handleSave = async () => {
    if (error) {
      message.error('发货净重只能是数字，且最多输入两位小数');
    } else {
      const params = {
        pId: record.id,
        weight: value * 1000,
...
const ToCheck = () => {
  ...
  const onSelectRow = (record, selected, selectedRows, nativeEvent) => {
    ...
    const toGoodsWeight = record.toGoodsWeight;
    ...
    let _check_toGoodsWeight = 0;
    let _check_loss = 0;
    _check_fromGoodsWeight += fromGoodsWeight || 0;
    _check_toGoodsWeight += toGoodsWeight || 0;
    _check_loss += loss || 0;
    ...
    if (selected) {
      setTotalCheck(total => {
        return {
          ...
          check_toGoodsWeight: total.check_toGoodsWeight + _check_toGoodsWeight,
          check_loss: total.check_loss + _check_loss,
        };
      });
    } else {
      setTotalCheck(total => {
  ...
  // 查询
  const getRemoteData = async ({ fromCompany, goodsType, plate, page, pageSize, begin, end }) => {
    setLoading(true);
  ...
  const handleAudit = async () => {
    ...
    if (res.status === 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {},
        onCancel: () => {
          sessionStorage.setItem('settlment_tab', 'to');
          router.back();
        },
      });
    } else {
        ...
        <section style={{ position: 'relative' }}>
          ...
          <Msg style={{ marginTop: 16 }}>
            ...
            <span style={{ marginRight: 32 }}>
              收货净重
              <span className="total-num">
                {Format.weight(isChecked || checkedAll ? total.sum_toGoodsWeight : totalCheck.check_toGoodsWeight)}
              </span>
              吨
            </span>
            <span style={{ marginRight: 32 }}>
              路损
              <span className="total-num">
                {Format.weight(isChecked || checkedAll ? total.sum_loss : totalCheck.check_loss)}
              </span>
              吨
            </span>
          </Msg>
