...
class View extends PureComponent {
  ...
  buildRecord = async () => {
    ...
    try {
      ...
      if (res.status === 0) {
        router.push(`/finance/invoiceList/record?id=${id}&&mode=${mode}&&batchId=${router.query.batchId}`);
      } else {
              ...
              {invoiceInfo && (
                <>
                  <Row style={{ color: '#333333' }}>
                    <Col span={8}>
                      <span style={{ color: '#848485' }}>企业名称：</span>
                      {invoiceInfo.companyName || '-'}
                    </Col>
                    <Col span={8}>
                      <span style={{ color: '#848485' }}>企业地址：</span>
                      {invoiceInfo.companyAddress || '-'}
                    </Col>
                    <Col span={8}>
                      <span style={{ color: '#848485' }}>企业电话：</span>
                      {invoiceInfo.invoicePhone || '-'}
                    </Col>
                  </Row>
                  <Row style={{ marginTop: 16, color: '#333333' }}>
                    <Col span={8}>
                      <span style={{ color: '#848485' }}>纳税人识别号：</span>
                      {invoiceInfo.taxpayerNumber || '-'}
                    </Col>
                    <Col span={8}>
                      <span style={{ color: '#848485' }}>账号：</span>
                      {invoiceInfo.cardNumber || '-'}
                    </Col>
                    <Col span={8}>
                      <span style={{ color: '#848485' }}>开户行：</span>
            ...
            <div style={{ float: 'right' }}>
              {this.state.status === 'REJECT_APPROVE' ? (
                <>
                  ...
                  <Button
                    style={{ marginLeft: 8 }}
                    onClick={() => router.push(`/finance/invoiceList/rejectList?batchId=${router.query.batchId}`)}>
                    修改数据
                  </Button>
                </>
              ) : (
