  ...
  return (
    <>
      ...
      <Msg>
        ...
        <span style={{ marginRight: 32, marginLeft: 8 }}>
          {/* 原发净重 */}
          原发净重<span className="total-num">{Format.weight(total.sum_fromGoodsWeight)}</span>吨
        </span>
        <span style={{ marginRight: 32, marginLeft: 8 }}>
          {/* 实收净重*/}
          实收净重<span className="total-num">{Format.weight(total.sum_goodsWeight)}</span>吨
        </span>
        <span style={{ marginRight: 32, marginLeft: 8 }}>
          {/* 原发净重总和 */}
        ...
        <span style={{ marginRight: 32, marginLeft: 8 }}>
          {/* 实收净重总和 */}
          路损<span className="total-num">{Format.weight(total.sum_loss)}</span>吨
        </span>
      </Msg>
      ...
      <DrawerInfo title="收货磅单补录" onClose={() => setShowDrawer(false)} showDrawer={showDrawer} width="933">
        <Supplement onSubmit={handleSearch} poundType="to" onClose={() => setShowDrawer(false)} />
      </DrawerInfo>
    </>
  );
};
