...
import { Modal, Spin, Input, Button, Pagination, message } from 'antd';
import s from './index.less';
import { Search } from '@components';
import { getRoute, getUseRoute } from '@api';
const Index = ({ handleSureChooseRoute, userId }) => {
  ...
  // 请求常用专线列表
  const useRouteList = async () => {
    setLoading(true);
    const { status, result, detail, description } = await getUseRoute({
      params: {
        receive: '0',
      },
    });
    ...
    if (!status) {
      setUseList(result.often);
    } else {
      message.error('专线获取失败');
    }
    setLoading(false);
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className={s['modal-content']} style={{ flex: 1, marginTop: 16 }}>
        <Spin spinning={loading}>
          <div style={{ marginBottom: 16, fontWeight: 500 }}>常用专线</div>
          <ul>
            {useList.map(item => {
              const {
                id,
          ...
          <div style={{ margin: '16px 0', fontWeight: 500 }}>专线列表</div>
          <ul>
            {routes.map(item => {
              const {
                id,
