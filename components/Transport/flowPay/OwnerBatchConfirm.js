...
const DetailStep = ({ totalGoodsWeight, totalArrivalGoodsWeight, transportCount, realPrice }) => {
  return (
    <div className={styles['detail-step']}>
      ...
      <div className={styles['data-row']}>
        结算运费：¥
        <span style={{ color: '#3D86EF', fontSize: 20, padding: '0 4px', fontWeight: 600 }}>
...
// 第二步
const PayStep = forwardRef(({ onChange, price }, ref) => {
  return (
...
const OwnerBatchConfirm = ({ payInfo, payId, onFinish, rides }) => {
  // 当前步骤
    ...
    <div className={styles['flow-pay']}>
      ...
      {step === 1 && (
        <div className={styles['step-block']} style={{ height: 163 }}>
          <PayStep onChange={setPassword} price={payInfo.realPrice} ref={passwordRef} />
...
export default OwnerBatchConfirm;
